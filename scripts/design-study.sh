...
port="4175"
study_path="/studies/red-c-green/"
local_url="http://127.0.0.1:${port}${study_path}"
public_url="https://${route}.grassinside.com${study_path}"
...
case "${1:-}" in
  up)
    ...
    systemd-run --user \
      ...
      --setenv=__VITE_ADDITIONAL_SERVER_ALLOWED_HOSTS="${route}.grassinside.com" \
      --working-directory="${project_dir}" \
      pnpm exec vite --config .impeccable/vite.study.config.mjs --host 127.0.0.1 --port "${port}" --strictPort >/dev/null
    wait_for_origin
