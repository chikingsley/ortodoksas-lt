...
const [
  ...
  reviewQueue,
    ...
    readJson("apps/web/public/content/catalog.json"),
    readJson("apps/web/public/media/manifest.json"),
    readJson("apps/web/public/media/assignments.json"),
    readJson("recovery/reports/media-ingestion.json"),
    readJson("recovery/reports/media-metadata-recovery.json"),
    readJson("recovery/reports/inline-media-recovery.json"),
    readJson("recovery/reports/wayback-media-recovery.json"),
    readJson("recovery/reports/body-media-recovery.json"),
...
try {
  ...
  for (const [method, report] of [
    ["article-position-alternates", inlineRecovery],
    ["wayback-cdx-exact-assets", waybackRecovery],
    ["facebook-object-and-direct-source", bodyRecovery],
  ]) {
    for (const item of report.recovered) {
      insertAttempt.run(
        ...
        method,
        "recovered",
        item.evidence ?? item.mediaPath ?? "exact asset recovered",
        JSON.stringify(item),
        report.generatedAt
      );
    }
    for (const item of report.unresolved) {
      const reasons = item.attempts ?? item.errors ?? [];
      insertAttempt.run(
        item.articlePath,
        item.originalUrl,
        method,
        "failed",
        reasons.at(-1) ?? "recovery method exhausted",
        JSON.stringify(item),
        report.generatedAt
      );
    }
