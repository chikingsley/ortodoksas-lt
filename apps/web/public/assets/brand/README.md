...
- Header identity: original horizontal `ortodoksas.lt` publication mark centered between social and language utilities
- Display type: Playfair Display
- Sans-serif type: Arimo
- Client palette: burgundy `#AF272F`, deep burgundy `#861D24`, gold `#C8A34A`, warm paper `#F4F1E8`, white, and black
...
The production interface uses the original horizontal Ortodoksas vector supplied by the identity owner. Its artwork, proportions, and official `#AF272F` color remain exact. Institutional Exarchate artwork remains available as source material for contexts that explicitly require the institution rather than the publication identity.

The Canva decorations remain pixel-faithful masters. The story-rail trace isolates one column from the supplied chain geometry for the approved narrow edge treatment. Preserve every ornament's orientation, proportions, and geometry. Production ornaments use the approved solid gold `#C8A34A` or official burgundy `#AF272F` according to their interface role.
...
The August 2026 client direction establishes official Ortodoksas burgundy `#AF272F`, dark burgundy `#861D24`, decorative gold `#C8A34A`, warm paper `#F4F1E8`, white, and black. The supplied logo guide identifies `#AF272F` as Pantone 1805 C. Burgundy owns functional emphasis; gold remains decorative.
...
Center the horizontal Ortodoksas mark in the public masthead. Preserve every mark's proportions and colors. Use institutional artwork only where the institution itself is the subject.
