...
import type { UploadedMedia } from "./image-upload-node-extension";
...
export interface FileItem {
  ...
  id: string;
  /**
   * Uploaded media metadata, available after a successful upload
   * @optional
   */
  media?: UploadedMedia;
  /**
...
function useFileUpload(options: UploadOptions) {
  ...
  const uploadFiles = async (files: File[]): Promise<UploadedMedia[]> => {
    ...
    return results.filter((media): media is UploadedMedia => media !== null);
  };
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
  ...
  const handleUpload = async (files: File[]) => {
    const mediaItems = await uploadFiles(files);

    if (mediaItems.length > 0) {
      const pos = props.getPos();

      if (isValidPosition(pos)) {
        const imageNodes = mediaItems.map((media, index) => {
          const filename =
          ...
          return {
            attrs: {
              ...extension.options,
              alt: media.altText,
              altProvenance: media.altTextProvenance,
              captionProvenance: media.captionProvenance,
              height: media.height,
              mediaId: media.id,
              src: media.url,
              title: filename,
            ...
            ...(media.caption
              ? {
                  content: [{ text: media.caption, type: "text" }],
                }
              : {}),
